...
/**
 * A node within a [ReversedRadixTree]. It is differs from [RadixTreeNode] in that the strings are stored in
 * reversed order. Contains logic to add and remove edges to other nodes.
 *
...
class ReversedRadixTreeNode(
     ...
     */
    private var edges: HashMap<Char, ReversedRadixTreeEdge>? = null
    ...
    override fun addEdge(
        ...
        return if(existingEdge == null){
            ReversedRadixTreeEdge(
                ...
                target = ReversedRadixTreeNode(isTerminal = true)
            ...
            if(splitIndex == -1){
                when{
                    ...
                    searchLabelLength > existingEdge.label.length ->
                        existingEdge.target.addEdge(label, start, end - existingEdge.label.length)
                    else -> {
                        val intermediateNode = ReversedRadixTreeNode(isTerminal = true)
                        val rootEdge = ReversedRadixTreeEdge(
                ...
                val intermediateNode = ReversedRadixTreeNode(isTerminal = false)
                val rootEdge = ReversedRadixTreeEdge(
